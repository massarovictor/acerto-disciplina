...
import { REPORT_COLORS, STATUS_COLORS, classifyStudent, StudentClassification } from '@/lib/reportDesignSystem';
...
export const StudentMetricsSlide = ({
  ...
  const metrics = useMemo(() => {
    ...
    const avgBySubject = subjects.map(sub => {
      ...
      return subGrades.length ? subGrades.reduce((s, g) => s + g.grade, 0) / subGrades.length : 0;
    });
  ...
  return (
    ...
    >
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 40, height: '100%' }}>
        {/* Left Column */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          <div style={{
            ...
            {student.photoUrl ? (
              <img
                ...
                alt={student.name}
                style={{ width: 140, height: 140, borderRadius: 70, objectFit: 'cover', border: `4px solid ${REPORT_COLORS.primary}` }}
              />
            ) : (
              <div style={{
                width: 140,
            ...
            <div>
              <p style={{ fontSize: 28, fontWeight: 700, color: REPORT_COLORS.text.primary, margin: '0 0 8px' }}>
                {student.name}
              </p>
              <p style={{ fontSize: 20, color: REPORT_COLORS.text.secondary, margin: 0 }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 32 }}>
              ...
              <div>
                <p style={{ fontSize: 22, color: metrics.statusColors.text, margin: '0 0 4px', fontWeight: 600, textTransform: 'uppercase', opacity: 0.8 }}>Média Geral</p>
                <p style={{ fontSize: 96, fontWeight: 800, color: metrics.statusColors.text, margin: 0, lineHeight: 1, letterSpacing: '-2px' }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 24, marginBottom: 24 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                  <p style={{ fontSize: 20, color: REPORT_COLORS.text.secondary, margin: 0, fontWeight: 600 }}>Posição</p>
                  <p style={{ fontSize: 48, fontWeight: 800, color: REPORT_COLORS.text.primary, margin: 0 }}>
                    {position}º <span style={{ fontSize: 24, color: REPORT_COLORS.text.tertiary, fontWeight: 500 }}>de {totalStudents}</span>
                  </p>
                </div>
              </div>
            </div>
            ...
            <p style={{ fontSize: 18, color: REPORT_COLORS.text.tertiary, margin: '16px 0 0', textAlign: 'right' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          {/* Incidents Card */}
          <div style={{
            background: metrics.studentIncidents.length === 0
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 32, marginBottom: 24 }}>
              ...
              />
              <div>
                <p style={{ fontSize: 20, color: REPORT_COLORS.text.secondary, margin: '0 0 4px', fontWeight: 600, textTransform: 'uppercase' }}>Ocorrências</p>
                <p style={{ fontSize: 64, fontWeight: 800, color: REPORT_COLORS.text.primary, margin: 0, lineHeight: 1 }}>
                  {metrics.studentIncidents.length}
                </p>
              </div>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 20, padding: '16px 24px', background: 'rgba(255,255,255,0.5)', borderRadius: 12 }}>
              <span style={{ color: REPORT_COLORS.text.secondary }}>Graves/Gravíssimas</span>
              <span style={{
                fontWeight: 700,
                fontSize: 24,
                color: metrics.criticalIncidents > 0 ? STATUS_COLORS.critical.text : REPORT_COLORS.text.primary
              }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 32 }}>
              ...
              <h3 style={{ fontSize: 24, fontWeight: 700, color: REPORT_COLORS.text.primary, margin: 0 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20, flex: 1, justifyContent: 'center' }}>
              <div style={{
                ...
                <span style={{ fontSize: 20, color: REPORT_COLORS.text.secondary }}>Total de Notas</span>
                <span style={{ fontSize: 28, fontWeight: 700, color: REPORT_COLORS.text.primary }}>{metrics.studentGrades.length}</span>
              </div>
              <div style={{
                display: 'flex',
                justifyContent: 'space-between',
                padding: '16px 20px',
                background: metrics.redGradesCount > 0 ? STATUS_COLORS.critical.bg : REPORT_COLORS.background.surface,
                borderRadius: 12,
                alignItems: 'center',
                border: metrics.redGradesCount > 0 ? `1px solid ${STATUS_COLORS.critical.border}` : 'none'
              }}>
                <span style={{ fontSize: 20, color: metrics.redGradesCount > 0 ? STATUS_COLORS.critical.text : REPORT_COLORS.text.secondary }}>Reprovadas (&lt; 6.0)</span>
                <span style={{
                  fontSize: 32,
                  fontWeight: 800,
                  color: metrics.redGradesCount > 0 ? STATUS_COLORS.critical.text : REPORT_COLORS.text.primary
                }}>
              ...
              <div style={{
                ...
                justifyContent: 'space-between',
                padding: '16px 20px',
                background: metrics.approvedGradesCount > 0 ? STATUS_COLORS.excellence.bg : REPORT_COLORS.background.surface,
                borderRadius: 12,
                alignItems: 'center',
                border: metrics.approvedGradesCount > 0 ? `1px solid ${STATUS_COLORS.excellence.border}` : 'none'
              }}>
                <span style={{ fontSize: 20, color: metrics.approvedGradesCount > 0 ? STATUS_COLORS.approved.text : REPORT_COLORS.text.secondary }}>Aprovadas (&gt;= 6.0)</span>
                <span style={{
                  fontSize: 32,
                  fontWeight: 800,
                  color: metrics.approvedGradesCount > 0 ? STATUS_COLORS.approved.text : REPORT_COLORS.text.primary
                }}>
          ...
          <div style={{
            ...
            <p style={{ fontSize: 18, color: REPORT_COLORS.text.secondary, margin: 0, textAlign: 'center', fontWeight: 500 }}>
              {metrics.averageGrade < 6 ? '🎯 Recomendação: Reforço escolar prioritário e reunião com pais.' :
                metrics.averageGrade < 7 ? '📚 Recomendação: Apoio contínuo nas matérias base.' :
                  metrics.averageGrade < 9 ? '🚀 Recomendação: Incentivar a monitoria e aprofundamento.' :
