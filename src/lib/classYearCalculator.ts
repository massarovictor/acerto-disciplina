 ...
 */
export function calculateCurrentYear(startYearDate: string, startYear: number): number {
  ...
  // Se ainda estamos no mesmo ano calendário mas passou o meio do ano letivo (fevereiro)
  // considerar que já avançou um ano letivo
  let academicYearsPassed = yearsDiff;
  if (yearsDiff === 0 && monthsDiff >= 2) {
    academicYearsPassed = 1;
  } else if (yearsDiff > 0 && monthsDiff >= 2) {
