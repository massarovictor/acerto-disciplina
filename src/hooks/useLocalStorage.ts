...
import { Incident, Class, Student, Grade, AttendanceRecord, ProfessionalSubjectTemplate } from '@/types';
import { MOCK_CLASSES, MOCK_STUDENTS } from '@/data/mockData';
import { generateSampleData } from '@/data/sampleData';
import { calculateCurrentYear, shouldArchiveClass } from '@/lib/classYearCalculator';

type StorageKey = 'INCIDENTS' | 'CLASSES' | 'STUDENTS' | 'GRADES' | 'ATTENDANCE' | 'PROFESSIONAL_SUBJECTS' | 'PROFESSIONAL_SUBJECT_TEMPLATES';
...
export function useLocalStorage<T>(key: StorageKey, initialValue: T) {
  ...
  const setValue = (value: T | ((val: T) => T)) => {
    try {
      ...
      console.log(`[useLocalStorage] ${key} atualizado. Novo total de itens:`, Array.isArray(valueToStore) ? valueToStore.length : 'N/A');
    } catch (error) {
...
export function useClasses() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (current && currentString !== lastKnownValue) {
        console.log('[useClasses] Detectada mudança no localStorage, atualizando estado');
  ...
  const generateClassNumber = (existingClasses: Class[]): string => {
    ...
    const exists = existingClasses.some(c => c.classNumber === formattedNumber);
    if (exists) {
  ...
  useEffect(() => {
    ...
    const updatedClasses = currentClasses.map(cls => {
      ...
      if (cls.startYearDate && cls.startYear && !cls.archived) {
        try {
          const calculatedYear = calculateCurrentYear(cls.startYearDate, cls.startYear);
          ...
          if (shouldArchiveClass(cls.startYearDate, cls.startYear) && !cls.archived) {
            updated.archived = true;
            updated.archivedAt = new Date().toISOString();
            updated.archivedReason = 'Arquivamento automático após completar 3 anos letivos';
  ...
  const updateClass = (id: string, updates: Partial<Class>) => {
    ...
    setClasses((prev) => {
      ...
      console.log('[useClasses] Total de turmas após atualização:', updated.length);
      return updated;
    });
  };
  ...
  const deleteClass = (id: string) => {
    ...
    setClasses((prev) => {
      ...
      console.log('[useClasses] Total de turmas após exclusão:', updated.length);
      return updated;
    });
  };
  ...
  return { classes, addClass, updateClass, deleteClass, archiveClass, unarchiveClass };
}
...
export function useProfessionalSubjects() {
  ...
  const [professionalSubjects, setProfessionalSubjects] = useLocalStorage<ProfessionalSubjectsByClass>(
  ...
  useEffect(() => {
    const handleStorageChange = () => {
      const updated = storage.get<ProfessionalSubjectsByClass>('PROFESSIONAL_SUBJECTS');
      if (updated) {
    ...
    const interval = setInterval(() => {
      const current = storage.get<ProfessionalSubjectsByClass>('PROFESSIONAL_SUBJECTS');
      const currentString = JSON.stringify(current);
  ...
  const setProfessionalSubjectsForClass = (classId: string, subjects: string[]) => {
    setProfessionalSubjects(prev => ({
      ...prev,
      [classId]: subjects,
    }));
...
export function useProfessionalSubjectTemplates() {
  const [templates, setTemplates] = useLocalStorage<ProfessionalSubjectTemplate[]>(
    'PROFESSIONAL_SUBJECT_TEMPLATES',
    []
  );

  const addTemplate = (template: Omit<ProfessionalSubjectTemplate, 'id' | 'createdAt' | 'updatedAt'>) => {
  ...
  const updateTemplate = (id: string, updates: Partial<ProfessionalSubjectTemplate>) => {
    setTemplates((prev) =>
      prev.map((template) =>
        template.id === id
